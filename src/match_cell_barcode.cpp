...
int main(int argc, char const *argv[])
{
  ...
  if (argc < 6)
  {
    std::cout << "usage: <1.fastq folder> <2.output cell barcode statistics file> <3.fastq output reads that matched cell barcode> <4.barcode reference from short read 10X data> <5.max edit distance> [6. UMI length (default: 10)] [7. Left-flanking sequence (default: \"CTACACGACGCTCTTCCGATCT\")]" << std::endl;
    exit(1);
  }
  else if (argc >= 6)
  {
  ...
  {
    std::cout << "set left reference to CTACACGACGCTCTTCCGATCT\n";
    ref_left = "CTACACGACGCTCTTCCGATCT";
  }
